"""
This module processes the CDS-bond basis final product.
"""
...
def process_cb_spread(df):
    """
    Process the CDS-bond spread data.

    df: dataframe with par spread values merged
       cusip, date, mat_days, BOND_YIELD, CS, size_ig, size_jk, par_spread

    output:
        additional columns:
        FR: Z-spread of the bond (FR = CS column)
        CB: implied return on CDS-bond spread (CB = par_spread - FR)
        rfr: implied risk free rate (rfr = (BOND_YIELD - CS) - CB)
...
def output_cb_final_products(df):
    """
    Output CDS-bond basis final products.

    df: dataframe with processed CDS-bond spread data

    output:
        agg_df: aggregated dataframe where data is combined by rating and date
...
def generate_graph(df, col="rfr"):
    ...
    ax1.set_ylabel("Implied Risk-Free Rate (percent)", color="black")

...
def load_crsp_treasury_for_fitting() -> pd.DataFrame:
    ...
    # The federated crsp_treasury repo names the dirty price "price_dirty";
    # finm's GSW filters / NSS fit expect a column literally named "price".
    if "price" not in df.columns and "price_dirty" in df.columns:
        df = df.rename(columns={"price_dirty": "price"})
